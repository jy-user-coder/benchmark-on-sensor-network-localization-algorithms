...
r_list = [0.125, 0.15, 0.175, 0.2, 0.225, 0.25]
num_test = 20
...
sgdhd_sols = np.zeros((d, n, len(r_list), 5*num_test))
...
for random_case in tqdm(range(5)):
    ...
    for i in tqdm(range(num_test)):
        ...
        for j, r in enumerate(r_list):
            ...
            x_ans_sgd_hd, loss_sgd_hd1, grad_sgd_hd1 = G.sgd_21_high_dim(s0, rng, tol=10**(-4), epochs=100, lr=0.5, batch=batch1, dr=0.9, penalty=0.0)
            ...
            x_ans_sgdcom, loss_sgdcom, grad_sgdcom = G.sgd_21(x_ans_sgd_hd, rng, tol=10**(-4), epochs=500, lr=0.01, batch=batch1, dr=0.99)
            rmsd_sgdcom = utils.RMSD(x_ans_sgdcom, sensors.T)
            sgdhd_sols[:, :, j, current_case] = x_ans_sgd_hd
